...
public class ParenthesizedVisitorTest extends AbstractRubyConverterTest {
  ...
  @Test
  public void begins_as_natives() throws Exception {
    Tree beginAsStatementList = rubyStatement("a; b;");
    assertTree(beginAsStatementList).isInstanceOf(NativeTree.class);
    assertThat(((NativeTree) beginAsStatementList).nativeKind()).isEqualTo(nativeKind("begin"));

    Tree kwBeginTree = rubyStatement("begin a end");
    assertTree(kwBeginTree).isInstanceOf(NativeTree.class);
    assertThat(((NativeTree) kwBeginTree).nativeKind()).isEqualTo(nativeKind("kwbegin"));
  }

  @Test
  public void not_expression_if_multiple_elements() throws Exception {
    Tree beginAsStatementList = rubyStatement("(a; b;)");
    assertTree(beginAsStatementList).isInstanceOf(NativeTree.class);
    assertThat(((NativeTree) beginAsStatementList).nativeKind()).isEqualTo(nativeKind("begin"));
  }
