  ...
  public List<Tree> getChildren(TreeMetaData metaData) {
    ...
    return children.stream()
      ...
      .map(child -> {
        ...
        Object differentiator = child;
        if (child instanceof RubySymbol) {
          differentiator = ((RubySymbol) child).asJavaString();
        }
        return new NativeTreeImpl(metaData, new RubyNativeKind(child, differentiator), Collections.emptyList());
      })
